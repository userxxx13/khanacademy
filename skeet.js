...
function setupMenu() {
    ...
    function setupWatermark() {
        ...
        watermark.innerHTML = `<span style="text-shadow: -1px 0.5px 0 #72ff72, -2px 0px 0 #2f672e;">Skeet</span> <span style="color:gray; padding-left:2px; font-family: Arial, sans-serif; font-size:10px">${ver}</span>`;
    ...
    function setupStatusPanel() {
        ...
        const update = async () => statsPanel.innerHTML = `
            <span style="text-shadow: -1px 0.5px 0 #p, -2px 0px 0 #2f672e;">Skeet</span>
...
function setupMain(){
    function spoofQuestion() {
        const phrases = [ "🔥 Get good, get Skeet!", "🤍 Made by bcz.", "☄️ By bcz. " ];
    ...
    function changeBannerText() {
        const phrases = [ "Non Skeetless dude.", "Skeet on top.", "bcz said hello!", "God i wish i had Skeet.", "Get good get Skeet!", "the old Skeet.c" ];
        setInterval(() => { 
...
async function autoAnswer() {
    const getRandomDelay = (baseDelay) => {
        const extraDelay = Math.random() * 60 + 30; // Atraso aleatório entre +30s e +1min
        return Math.min(baseDelay + extraDelay, 300); // Máximo de 5 minutos
    };
    ...
    const baseDelay = featureConfigs.autoAnswerDelay; // Em segundos
    const randomDelay = getRandomDelay(baseDelay) * 1000; // Converter para milissegundos
    console.log(`Próxima resposta em: ${randomDelay / 1000}s`);
...
/* Inject */
if (!/^https?:\/\/pt\.khanacademy\.org/.test(window.location.href)) { alert("❌ Skeet Failed to Injected!\n\nVocê precisa executar o Skeet no site do Khan Academy! (https://pt.khanacademy.org/)"); window.location.href = "https://pt.khanacademy.org/";};
...
.then(async () => {
    sendToast("🌿 Skeet injetado com sucesso!");
